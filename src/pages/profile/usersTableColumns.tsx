...
import { formatDate, formatName } from "@/components/utils";
...
import { UserModel, Users } from "@/components/types";
...
interface Props {
  users: Users | undefined; // Ensure custOrders is optional or handled
}
...
export function UserTable({ users }: Props) {
  ...
  const table = useReactTable({
    data: users?.content || [], // Provide an empty array as fallback
    columns,
    getCoreRowModel: getCoreRowModel(),
  });

  if (!users || !users.content || users.content.length === 0) {
    return <div>No users to display.</div>; // Or some other appropriate message
  }
